...
def determine_attenuation(wave,data_flux,error_flux,model_flux,SPM,age,metal):
	...
	hpf_data    = hpf(data_flux)#; print(hpf_data); py.plot(wave,data_flux); py.plot(wave,hpf_data); py.show(); sys.exit()
	...
	smooth_attenuation = curve_smoother(wave,fine_attenuation,smoothing_length)#;py.plot(wave,fine_attenuation);py.plot(wave,smooth_attenuation)
