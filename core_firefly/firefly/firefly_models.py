...
class StellarPopulationModel:
			...
			for zi,z in enumerate(metal_files):
				...
				for a in age_data:
					...
					if (((10**(logyrs_a-9)) < self.age_limits[0]) or ((10**(logyrs_a-9)) > self.age_limits[1])):
						...
						first_model = False
			#print(model_flux[0][100]);print(age_model[0]);print(np.log10(metal_model[0]))
			#sys.exit()
	...
	def fit_models_to_data(self):
		...
		for mi,mm in enumerate(self.model_libs):
			...
			print("-----"); print('Determining dust attenuation...')
			if self.hpf_mode=='on':
				...
				print("-----"); print('Fitting models to data...')
				...
				#print('dust done, Dt=', np.round(time.time()-t_i,decimals=0),'seconds')
				# 4. Fits the models to the data
				light_weights, chis, branch = fitter(wave, hpf_data, hpf_error, hpf_models, self)
				#print('fitting done, Dt=', np.round(time.time()-t_i,decimals=0),'seconds')

			print("-----"); print('Retrieving the best fit model...')
			...
			if np.all(np.isnan(mass_weights)):
				...
				waveCol_um = pyfits.Column(name="wavelength_um",format="D", unit="Angstrom", array= model_wave_int)
				best_fitCol_um = pyfits.Column(name="firefly_model_um",format="D", unit="1e-17erg/s/cm2/Angstrom", array= best_fit_full)
				...
				#cols = pyfits.ColDefs([waveCol, dataCol, errorCol, best_fitCol, waveCol_um, best_fitCol_um, best_fitCol_um_noHPF]) # dataCol, errorCol, waveCol, best_fitCol,
				cols = pyfits.ColDefs([waveCol, dataCol, errorCol, best_fitCol]) # dataCol, errorCol, waveCol, best_fitCol,
